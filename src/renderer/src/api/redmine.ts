...
export class RedmineApi {
  ...
  async getAllMyOpenIssues(offset = 0, limit = 100): Promise<TIssue[]> {
    return this.instance
      .get(
        `/issues.json?offset=${offset}&limit=${limit}&status_id=open&assigned_to_id=me&sort=updated_on:desc`
      )
  ...
  async getOpenIssuesByIds(ids: number[], offset = 0, limit = 100): Promise<TIssue[]> {
    return this.instance
      .get(
        `/issues.json?offset=${offset}&limit=${limit}&status_id=open&issue_id=${ids.join(',')}&sort=updated_on:desc`
      )
      .then((res) => res.data.issues)
  }
  ...
  async getOpenIssuesByProject(projectId: number, offset = 0, limit = 100): Promise<TIssue[]> {
    return this.instance
      .get(
        `/issues.json?offset=${offset}&limit=${limit}&status_id=open&project_id=${projectId}&sort=updated_on:desc`
      )
      .then((res) => res.data.issues)
  }
  ...
  async searchOpenIssues(query: string): Promise<TSearchResult[]> {
    return this.instance
      .get(`/search.json?q=${query}&scope=my_project&titles_only=1&issues=1&open_issues=1`)
  ...
  async getIssuePriorities(): Promise<TIssuesPriority[]> {
    return this.instance
      ...
      .then((res) => res.data.issue_priorities)
  }

  // Projects
  async getAllMyProjects(offset = 0, limit = 100): Promise<TProject[]> {
    return this.instance
      .get(`/projects.json?offset=${offset}&limit=${limit}`)
      .then((res) => res.data.projects)
  }

  async searchProjects(query: string): Promise<TSearchResult[]> {
    return this.instance
      .get(`/search.json?q=${query}&scope=my_project&titles_only=1&projects=1`)
      .then((res) => res.data.results)
  }

  async getProjectMemberships(id: number, offset = 0, limit = 100): Promise<TMembership[]> {
    return this.instance
      .get(`/projects/${id}/memberships.json?offset=${offset}&limit=${limit}`)
  ...
  async getAllMyTimeEntries(from: Date, to: Date, offset = 0, limit = 100): Promise<TTimeEntry[]> {
    return this.instance
      .get(
        `/time_entries.json?offset=${offset}&limit=${limit}&user_id=me&from=${formatISO(from, { representation: 'date' })}&to=${formatISO(to, { representation: 'date' })}`
      )
      ...
      .post(`/time_entries.json`, {
        time_entry: {
          ...
          spent_on: entry.spent_on
            ? formatISO(entry.spent_on, { representation: 'date' })
            : undefined
        }
      })
      .then((res) => res.data)
  ...
  async getTimeEntryActivities(): Promise<TTimeEntryActivity[]> {
    return this.instance
      ...
      .then((res) => res.data.time_entry_activities)
  }
