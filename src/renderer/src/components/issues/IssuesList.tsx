...
const IssuesList = ({
  ...
  issuePriorities,
  projectVersions,
  issuesData: { data: issuesData, setData: setIssuesData },
  onSearchInProject
}: PropTypes) => {
  ...
  const groupedIssues = getGroupedIssues(
    getSortedIssues(
      rawIssues,
      settings.style.sortIssuesByPriority ? issuePriorities.data : [],
      issuesData
    ),
      ...
      {groupedIssues.map(({ type, project, versions, groups }) => (
        <Fragment key={type}>
          {project && (
            <div
              className={clsx('flex justify-between gap-x-2', {
                'sticky top-0 z-[5] -mx-2 -my-1 bg-background px-2 py-1 shadow shadow-background':
                  settings.style.stickyScroll
              })}
            >
              <a
          ...
          {groups.map(({ type, version, issues }) => (
            <>
              {settings.style.groupIssuesByVersion &&
                versions.length > 0 &&
                ['version', 'no-version'].includes(type) && (
                  <>
                    {version && <VersionTooltip version={version} />}
                    <div
                      className={clsx({
                        'shadow- sticky top-6 z-[5] -mx-2 -my-1 bg-background px-2 py-1 shadow shadow-background':
                          settings.style.stickyScroll
                      })}
                    >
                      <span
                      ...
                      >
                        {type === 'version' && version && (
                          <a
                            href={`${settings.redmineURL}/versions/${version.id}`}
                            target="_blank"
                            tabIndex={-1}
