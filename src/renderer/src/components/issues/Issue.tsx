...
const Issue = ({
  ...
  return (
    <>
            ...
            [
              {
                name: formatMessage({
                  id:
                    assignedToMe || remembered
                      ? 'issues.context-menu.pin'
                      : 'issues.context-menu.pin-and-remember'
                }),
            ...
            className={clsxm(
              ...
              settings.style.showIssuesPriority
                ? {
                    ...
                    'border-2 border-[#ffb4b4] dark:border-[#ff5050]/40':
                      priorityType === 'higher' || priorityType === 'highest'
                  }
            ...
            <h1
              className={clsx(
                ...
                settings.style.showIssuesPriority && {
                  ...
                  'text-[#900] dark:text-[#fa7070]':
                    priorityType === 'high' || priorityType === 'higher',
            ...
            <div className="flex flex-row justify-between gap-x-2">
              <div className="mt-1">
                <div className="w-[80px] bg-[#eeeeee]">
                  <div
                    className="select-none bg-[#bae0ba] p-1 text-center text-xs font-medium leading-none text-gray-600"
            ...
            <div className="absolute right-2 top-2 flex items-start justify-end gap-x-1">
              {pinned && (
                <>
                  {settings.style.showTooltips && (
                    <Tooltip
                      id={`tooltip-pinned-${issue?.id}`}
                      place="left"
                      delayShow={700}
                      content={formatMessage({ id: 'issues.issue.pinned' })}
                      className="italic"
                    />
                  )}
                  <FontAwesomeIcon
                    icon={faThumbTack}
                    className="dark:text-white-600 rotate-[30deg] text-gray-300 focus:outline-none"
                    data-tooltip-id={`tooltip-pinned-${issue?.id}`}
                    tabIndex={-1}
                  />
                </>
              )}
              {!assignedToMe && (
                <>
                  {settings?.style?.showTooltips && (
                    <Tooltip
                      id={`tooltip-not-assigned-to-me-${issue?.id}`}
                      place="left"
                      delayShow={700}
                      content={formatMessage({ id: 'issues.issue.not-assigned-to-me' })}
                      className="italic"
                    />
                  )}
                  <FontAwesomeIcon
                    icon={faCircleUser}
                    className="text-gray-300 focus:outline-none dark:text-gray-600"
                    data-tooltip-id={`tooltip-not-assigned-to-me-${issue?.id}`}
                    tabIndex={-1}
                  />
                </>
              )}
      ...
      {settings?.style?.showTooltips && (
        <Tooltip
          id={`tooltip-toggle-timer-${issue?.id}`}
          place="bottom"
          delayShow={4000}
          className="z-10 max-w-[275px] italic"
        >
          <FormattedMessage
      ...
      {copiedIdToClipboard && (
        <Toast
          type="success"
          message={formatMessage({ id: 'issues.id-copied-to-clipboard' }, { issueId: issue?.id })}
          autoClose={2500}
          onClose={() => setCopiedIdToClipboard(false)}
        />
      )}
    </>
  )
