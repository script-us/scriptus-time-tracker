...
const CreateTimeEntryModal = ({ issue, time, isPause, onClose, onSuccess }: PropTypes) => {
  ...
  const users = useProjectUsers(issue.project.id, {
    enabled: settings.features.addSpentTimeForOtherUsers
  })

  const cachedComments = useStorage<Record<number, string | undefined>>(
    'cachedComments',
    _defaultCachedComments
  )

  useEffect(() => {
    formik.current?.setFieldValue(
      'activity_id',
      timeEntryActivities.data.find((entry) => entry.is_default)?.id ?? undefined
    )
      ...
      >
        <Formik
          ...
          validationSchema={Yup.object({
            ...
            hours: Yup.number()
              .required(
                formatMessage({ id: 'issues.modal.add-spent-time.hours.validation.required' })
              )
              .min(
                0.01,
                formatMessage({
                  id: 'issues.modal.add-spent-time.hours.validation.greater-than-zero'
                })
              )
              .max(
                24,
                formatMessage({ id: 'issues.modal.add-spent-time.hours.validation.less-than-24' })
              ),
            spent_on: Yup.date().max(
              new Date(),
              formatMessage({ id: 'issues.modal.add-spent-time.date.validation.in-future' })
            ),
            user_id: Yup.array(Yup.number()),
            comments: Yup.string(),
            activity_id: Yup.number().required(
              formatMessage({ id: 'issues.modal.add-spent-time.activity.validation.required' })
            ),
          ...
          onSubmit={async (originalValues, { setSubmitting }) => {
            ...
            if (values.done_ratio !== issue.done_ratio || (values.add_notes && values.notes)) {
              await updateIssueMutation.mutateAsync({
                done_ratio: values.done_ratio !== issue.done_ratio ? values.done_ratio : undefined,
                notes: values.add_notes ? values.notes : undefined
              })
            }
              ...
              <div className="flex flex-col gap-y-2">
                ...
                <div className="flex justify-between gap-x-3">
                  ...
                  {issue.estimated_hours && (
                    <SpentVsEstimatedTime
                      issue={issue}
                      previewHours={values.hours ? values.hours : 0}
                    />
                  )}
                </div>
                ...
                {values.spent_on && (
                  <TimeEntryPreview
                    date={startOfDay(values.spent_on)}
                    previewHours={values.hours ? values.hours : 0}
                  />
                )}
                ...
                <Fieldset className="flex flex-col gap-y-2">
                  ...
                  {settings.features.addSpentTimeForOtherUsers && (
                    <FastField
                      ...
                      noOptionsMessage={() =>
                        formatMessage({ id: 'issues.modal.add-spent-time.user.no-users' })
                      }
                      ...
                      options={groupedUsers.map(({ role, users }) => ({
                        ...
                        options: users.map((user) => ({
                          value: user.id,
                          label:
                            user.id === myAccount.data?.id
                              ? `${user.name} (${formatMessage({ id: 'issues.modal.add-spent-time.user.me' })})`
                              : user.name
                        }))
                      }))}
                ...
                {settings.features.addNotes &&
                  (!values.add_notes ? (
                    <FastField
                      type="checkbox"
                      name="add_notes"
                      title={formatMessage({ id: 'issues.modal.add-spent-time.add-notes' })}
                      as={Toggle}
                      error={touched.add_notes && errors.add_notes}
                    />
                  ) : (
                    <Fieldset
                      legend={formatMessage({ id: 'issues.modal.add-spent-time.notes' })}
        ...
        <Toast
          ...
          message={
            isAxiosError(createTimeEntryMutation.error)
              ? (
                  createTimeEntryMutation.error as AxiosError<TRedmineError>
                ).response?.data?.errors?.join(', ') ??
                (createTimeEntryMutation.error as AxiosError).message
