import { faPause, faPlay, faRedoAlt, faPlus } from '@fortawesome/free-solid-svg-icons'
...
const IssueTimer = forwardRef(
  (
    {
      issue,
      data: { active, time, start },
      onStart,
      onPause,
      onReset,
      onOverrideTime,
      onDoneTimer,
      setIsPause    }: PropTypes,
    ...
    return (
      <>
        <div className="flex items-center justify-end gap-x-3">
          {(editMode && (
            ...
            <>
              {settings?.style?.showTooltips && (
                <Tooltip
                  id={`tooltip-edit-timer-${issue?.id}`}
                  place="top"
                  delayShow={700}
                  content={formatMessage({ id: 'issues.timer.action.edit.tooltip' })}
                  className="italic"
                />
              )}
              <span
                className={clsx(
                  'text-lg',
                  timer > 0 ? 'text-yellow-500' : 'text-gray-700 dark:text-gray-500',
                  active && 'font-bold'
                )}
          ...
          {!active ? (
            <>
              {settings?.style?.showTooltips && (
                <Tooltip
                  id={`tooltip-start-timer-${issue?.id}`}
                  place="left"
                  delayShow={700}
                  className="italic"
                >
            ...
            <>
              {settings?.style?.showTooltips && (
                <Tooltip
                  id={`tooltip-pause-timer-${issue?.id}`}
                  place="left"
                  delayShow={700}
                  className="italic"
                >
              ...
              <FontAwesomeIcon
                ...
                onClick={(e) => {
                  ...
                  setIsPause(true)
                  onDoneTimer(
                    settings?.features?.roundTimeNearestQuarterHour
                      ? roundTimeNearestQuarterHour(timer)
                      : timer
                  )
                }}
          ...
          {settings?.style?.showTooltips && (
            <Tooltip
              id={`tooltip-reset-timer-${issue?.id}`}
              place="top"
              delayShow={700}
              content={formatMessage({ id: 'issues.timer.action.reset.tooltip' })}
              className="italic"
            />
          )}
          <FontAwesomeIcon
          ...
          {settings?.style?.showTooltips && (
            <Tooltip
              id={`tooltip-done-timer-${issue?.id}`}
              place="bottom"
              delayShow={700}
              content={formatMessage({ id: 'issues.timer.action.add-spent-time.tooltip' })}
              className="z-10 italic"
            />
          )}
          <FontAwesomeIcon
            role="button"
        ...
        {confirmResetModal && (
          <Modal
            title={formatMessage({ id: 'issues.modal.reset-timer.title' })}
