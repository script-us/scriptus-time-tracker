...
>({ title, error, className, ...props }: Props<Option, IsMulti, Group> & PropTypes) {
  return (
    <div className={className}>
      {title && (
        <label className="mb-1 block text-sm font-medium">
          {title}
          {props.required && (
            <FontAwesomeIcon icon={faAsterisk} size="2xs" className="ml-1 text-red-600" />
          )}
        </label>
      )}
        ...
        components={{
          ...
          Option: (optionProps) => (
            ...
            <components.Option {...optionProps}>
              ...
              {(optionProps.data as any).icon && (
                <span className="mr-2 inline-block w-5">{(optionProps.data as any).icon}</span>
              )}
              {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
          ...
          SingleValue: (singleValueProps) => (
            ...
            <components.SingleValue {...singleValueProps}>
              {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
              {(singleValueProps.data as any).icon && (
                <span className="mr-2">{(singleValueProps.data as any).icon}</span>
              )}
              {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
        ...
        classNames={{
          control: (state) =>
            clsx('block w-full rounded-lg border border-field-border bg-field p-2 text-sm', {
              ...
              'border-red-500 text-red-900 dark:border-red-500 dark:text-red-500':
                error !== undefined
            }),
          placeholder: () =>
          ...
          multiValue: () =>
            'rounded-md bg-field-inner px-2 py-1 text-xs font-medium text-gray-600 dark:text-gray-300 ring-1 ring-inset ring-gray-600/10 dark:ring-gray-300/10 max-w-[240px]',
