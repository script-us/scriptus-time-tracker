...
export const getSortedIssues = (
  issues: TIssue[],
  issuePriorities: TIssuesPriority[],
  issuesData: IssuesData
) => {
  const issuePrioritiesIndices = issuePriorities.reduce(
    (result: Record<number, number>, priority, index) => {
  ...
  const grouped = issues.reduce((result: GroupedIssuesHelper, issue) => {
    ...
    if (issue?.fixed_version && projectVersions[issue?.project?.id]) {
      if (!(issue?.fixed_version.id in result[issue?.project?.id].versions)) {
        const version = projectVersions[issue?.project?.id].find(
          (v) => v?.id === issue?.fixed_version?.id
        )
