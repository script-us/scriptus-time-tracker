...
const useMyIssues = (additionalIssuesIds: number[], search?: SearchQuery, filter?: FilterQuery) => {
  ...
  const extendedSearching = search?.searching && ((debouncedSearch as string).length >= 3 || extendedSearchIssueIdMatch !== null) && settings?.features?.extendedSearch;

  const extendedSearchIssuesResultQuery = useQuery({
    queryKey: ["extendedSearchIssuesResult", debouncedSearch],
    queryFn: () => redmineApi.searchOpenIssues(debouncedSearch as string),
    enabled: extendedSearching && !(debouncedSearch as string).includes("#"),
    placeholderData: keepPreviousData,
    staleTime: -1,
  });
  ...
  // filter: projects
  if ((filter?.projects as number[])?.length > 0) {
