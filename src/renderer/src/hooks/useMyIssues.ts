...
const useMyIssues = (additionalIssuesIds: number[], search: SearchQuery, filter: FilterQuery) => {
  ...
  const issuesQuery = useInfiniteQuery({
    ...
    getNextPageParam: (lastPage, allPages) =>
      lastPage?.length === 100 ? allPages?.length : undefined,
  ...
  const additionalIssuesQuery = useInfiniteQuery({
    ...
    initialPageParam: 0,
    queryFn: ({ pageParam }) =>
      redmineApi.getOpenIssuesByIds(additionalIssuesIds, pageParam * 100, 100),
    getNextPageParam: (lastPage, allPages) =>
      lastPage.length === 100 ? allPages.length : undefined,
  ...
  useEffect(() => {
    if (additionalIssuesQuery.hasNextPage && !additionalIssuesQuery.isFetchingNextPage)
      additionalIssuesQuery.fetchNextPage()
  }, [additionalIssuesQuery])

  let issues = issuesQuery.data?.pages?.flat() ?? []
  issues.push(
    ...(additionalIssuesQuery.data?.pages
      ?.flat()
      .filter((issue) => !issues.find((iss) => iss.id === issue.id)) ?? [])
  )
  ...
  if (search.searching && search.query) {
    issues = issues.filter((issue) =>
      new RegExp(search.query, 'i').test(`#${issue.id} ${issue.subject}`)
    )
  }
  ...
  const extendedSearching =
    search.searching &&
    (debouncedSearch.length >= 3 || extendedSearchIssueIdMatch !== null) &&
    settings.features.extendedSearch
  ...
  const extendedSearchIssuesResultIds = (
    extendedSearchIssuesResultQuery.data?.map((result) => result.id) ?? []
  ...
  const extendedSearchIssuesQuery = useQuery({
    ...
    queryFn: () =>
      redmineApi.getOpenIssuesByIds(
        extendedSearchIssuesResultIds,
        0,
        search.inProject ? 100 : MAX_EXTENDED_SEARCH_LIMIT
      ),
  ...
  const extendedSearchIssuesList =
    (search.inProject
      ? extendedSearchIssuesQuery.data?.filter((issue) => issue.project.id === search.inProject?.id)
      : extendedSearchIssuesQuery.data
    )?.slice(0, MAX_EXTENDED_SEARCH_LIMIT) ?? []
  ...
  if (filter.projects.length > 0) {
    ...
    extendedSearchIssues = extendedSearchIssues.filter((issue) =>
      filter.projects.includes(issue.project.id)
    )
  }
